...
public class RTree<T extends Shape> implements Writable, Iterable<T> {
  ...
  /*xuxiaomin: added for whaledb usage*/
  public RTree(T stockObj, boolean need_ld_to_mem)
  {
	  this.stockObject = stockObj;
	  set_is_inmemory(false);
	  set_need_load_to_memory(need_ld_to_mem);
  }
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    if (in instanceof FSDataInputStream) {
    	...
    	if(!need_load_to_memory)
    	{
    	      this.treeStartOffset = 0 - structureSize - TreeHeaderSize;
    	      this.data = null; /*clear it again*/
    	      this.data = (FSDataInputStream) new FSDataInputStream((InputStream) in);
    	      is_inmemory = false;
    	}
    	else
    	{
  ...
  protected boolean is_inmemory, need_load_to_memory;
  
  protected void set_is_inmemory(boolean v){
	  is_inmemory = v;
  }
  
  protected void set_need_load_to_memory(boolean v){
	  need_load_to_memory = v;
  }
  
  /**
   * functionality: switch the storage of R-tree between hdfs-based and mem-based
   * @param in
   * @param need_load_tomen
   * @throws IOException
   */
  public void setRtreeStorage(DataInput in,boolean need_load_tomen) throws IOException
  {
	  need_load_to_memory = need_load_tomen;
	  readFields(in);
  }
    ...
    while (!toBeSearched.isEmpty()) {
      ...
      if (searchNumber < nodeCount) {
        ...
        while (firstOffset < lastOffset) {
          ...
          if(stockObject == null)
          {
        	  if(stockObject instanceof Rectangle)
        	  {
        		  stockObject = (T) new Rectangle();
        	  }
        	  else
        	  {
        		  stockObject = (T) new Rectangle();
